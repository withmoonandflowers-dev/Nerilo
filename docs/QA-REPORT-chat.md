...
- ~~同代異鑰靜默覆寫：同一房間金鑰 epoch 收到兩把不同金鑰時後到覆寫先到、
  不觸發補顯示、無日誌線索，房間靜默分裂且不自癒。~~ **已修（2026-08-24，
  Spec 022）**：金鑰環改記產生方身分（已簽章綁定的 senderId），同代異鑰改為
  先到勝出＋衝突記錄＋每 epoch 一次告警＋主動請求換代（每會話上限 3 次，
  用盡＝需人介入且狀態可查）。查詢介面 `getKeyConflicts()`；Vue 聊天頁常駐
  橫幅（key-conflict-notice）。殘留（4.1 拍板）：不比對金鑰材料本身，
  「同一產生方同代發不同鑰」偵測不到——該情況由協調器 distributedRosterSig
  冪等守衛擋住。回歸：RoomContentKeysConflict.spec 6 例＋RoomKeyCoordinator
  換代閉環 1 例。

- ~~`verifyMessage` 5 分鐘時效窗：超過 5 分鐘的訊息補送給遲到者會被驗簽拒絕
  （長會話遲入者收不到舊訊息）。獨立工作項。~~ **已修（2026-07-05）**：gossip
  收訊路徑驗簽改傳 `maxAgeMs: null` 停用時效窗，補送與首次洪泛在線路上無法
  區分（補送即原始已簽名訊息重送），任何 wall-clock 門檻都會拒掉補送；該路徑
  重放防護由 (senderId, seq) store 去重 + floor 承擔，未來時間戳不受影響、仍
  一律拒絕。回歸：`SecurityManager.spec` 時效窗可停用 4 例、
  `GossipMessageHandler.spec` 真簽章補送 30 分鐘舊訊息整合測試。
  ~~**殘留風險（誠實記錄）**：跨會話重放不再受 5 分鐘窗限制，新會話 store 為
  空，攻擊者（需為房內連上的鄰居）可重放舊會話錄下的合法訊息，使舊內容重現
  或預佔 seq 槽位。緩解需 session epoch 納入簽章，屬獨立工作項。~~
  **已修（2026-07-18，Spec 009）**：sessionEpoch 入簽章＋收端 per-sender 現行代
  門檻（低於已知現行代一律完全拒收：不入 store、不上 UI、不轉發）；去重鍵與
  digest 全面分代 (senderId, sessionEpoch, seq)，「預佔槽位」在任何情況下結構上
  不可能。現行代持久化（IndexedDB），同裝置重載即拒舊代。回歸：
  `GossipMessageHandler.spec` V1 重放與預佔劇本、`SessionEpochConformance.spec`
  C1-C7 真簽章向量；30 分鐘補送回歸鎖維持綠（epoch 門檻與 wall-clock 正交）。
  **Spec 009 後的殘留（誠實記錄）**：
  - 全新 store 的收端在「尚未觀察到某 sender 現行代」的首接觸窗內，舊會話重放
    仍會被當現行接受（有界：一旦看到更高代即採納並拒後續舊代；佔位不可能）。
    這是自宣告代的固有限制，完全消除需外部權威，屬未來項。
  - 跨會話補歷史能力放棄（使用者拍板 Q2）：新成員／新裝置補不到舊會話（非現行
    代）的歷史；同會話內 anti-entropy 補送完好。
  - 清儲存＋系統時鐘倒退並存的 sender，其新代可能低於他人已採納的代 → 被拒收
    直到時鐘越過舊代（fail-closed 方向，不破壞不變量）。
  - 信使寄存槽位仍以 (senderId, seq) 為鍵（Spec 003/004 持久 schema 未動）：
    換裝置後同 seq 的新代紀錄在仍持有舊代同槽紀錄的信使處會被拒收，只損備份
    冗餘，不損恰好一次；靠信使 TTL／墓碑自然清退。
  - v1↔v2 混版房不互通（拍板 Q3 不設降級通道）：GOSSIP_HELLO 版本訊號＋缺
    sessionEpoch 訊息確證 → 兩線 UI fail-visible 提示「請雙方更新」。
- 星型→mesh 遷移期間送出的訊息屬星型棧，mesh 對帳管不到；診斷測試在
  mesh 模式訊號（`.e2ee-indicator-dtls`）後才發送。**處置（2026-07-18，
  Spec 010 拍板）**：範圍限 React 產線，Vue 接班線已依 ADR-0023 P2-③
  退役星型棧（2 人房從第一則訊息就走 gossip 複寫日誌），設計上無此窗，
  並有回歸鎖 `tests/e2e-vue/migration-window.spec.ts`（第三人加入的同時
  三方連發、矩陣全 =1、不等 mesh 就緒訊號）釘成測試事實。React 線不修
  （凍結中、即將退役），記為誠實邊界，由 Vue 切 production 收斂；回歸鎖
  已列入 ADR-0017 切換門檻。切換完成前 React 產線維持此限制（遷移窗可能
  無聲掉信、遲到者看不到星型時代歷史）。
- ~~實務上限 5 人（partial mesh 未接線）~~ **已接線（2026-07-18，Spec 011／
  ADR-0035）**：第 7 人起 partial mesh（k=max(3,⌈√n⌉)、ttl 3）、只升不降、
  房間上限 10（Free 5／Pro 10）。證據：n=7..10 模擬 1100 組 seed（churn＋晚到者）
  全收斂；7 人 E2E spec 已落地，單機高負載下矩陣未穩定轉綠（Spec 011 V1 誠實記錄）。
  **2026-08-02 更新（Spec 016／ADR-0037）**：低負載重跑推翻「純 CPU 排擠」解釋，
  根因是成形層互選機率化＋signaling 訂閱視窗溢出，已修（確定性互選＋入站應答＋
  視窗 50→400），成形層模擬 1600 seed 落地；2026-08-03 收口：CI e2e-7p R6/R7/R8 連續 3 綠（另修 Spec 018 產生方交接撞代與
  測試 harness 兩處自傷）。殘留：分島自癒（Spec 016 殘留 D）為獨立工項。**殘留（誠實記錄）**：旋轉 churn 拆線窗內的訊息依賴
  對帳補齊，與遷移窗同族，實測若暴露掉信轉交該工作項；橋接條件改
  connected<min(n-1,k)，「鄰居健全≠全房可達」的盲點由模擬連通性劇本看住。
- ~~備援橋接為明文（mesh 房本無 E2EE，UI 已誠實標示）~~ **已修（2026-07-18，
  Spec 012）**：mesh 房 gossip 內容自 ADR-0023 P2 起密文化；Spec 012 進一步關閉
