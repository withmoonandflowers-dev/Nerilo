...
3. **待做（最高剩餘 CP 值）**：接 **Stryker mutation** 到
   `RecordCrypto`/`antiEntropy`/`SecurityManager`/`GossipMessageHandler`，
   score 目標 ≥ 80%，補測殺存活 mutant。
4. **待做**：強化弱斷言——加密相關的 `toBeTruthy` 改成「密文≠明文 + 可解回」。
5. ✅ **續守**：E2E 誠實條款（exactly-once 矩陣 + 不准加長 timeout 硬湊綠）。
