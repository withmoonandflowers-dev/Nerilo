...
- 方法：對齊 `docs/testing-discipline.md`（覆蓋率地板、property、mutation、模擬）
...
3. ✅ **已做**：接 **Stryker mutation**（`stryker.conf.mjs`，`npm run test:mutation`）。
   `antiEntropy.ts` 首跑 78.70%，mutation 找出 normalizeDigest 驗證分支 + peerLacks
   floor 守衛未測 → 補 `antiEntropy.validation.spec.ts` 殺存活者 → **87.96%（>80% 目標）**。
   剩餘 survivors 多為等價 mutant。RecordCrypto 已列入 config，可 on-demand 跑。
...
| **mutation（Stryker）** | ✅ antiEntropy 87.96% |
