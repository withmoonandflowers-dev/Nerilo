...
把端到端加密、點對點直連、離線代存送達的即時通訊，嵌進你自己的產品。你只依賴 `NeriloClient` 這個門面，內部的 mesh、gossip、密碼學細節都封裝掉，可自由重構而不動你的程式。
...
各情境可用性（與對外宣稱同一口徑，不多不少）：

| 情境 | 可用嗎 | 用什麼 |
|---|---|---|
| 網路正常 | 可 | 預設 Firestore 會合 |
| 對外斷線、區域網還在（社區路由器/熱點） | **可** | 本地會合點 |
| 什麼基礎設施都沒有、兩人面對面 | **可** | 離線邀請碼（雙向雙 QR） |
| 斷網前已連上的 mesh | 可 | 不需任何動作，P2P 與信使照常 |
| 相隔兩地且兩邊都全斷 | 不可 | 需異質傳輸（LoRa，研究中） |

**本地會合點**：區網任一台裝置跑 `npx nerilo-rendezvous`（零依賴、無狀態、行程結束即消失），
瀏覽器端注入 `createHttpSignaling('http://<區網IP>:9473')` 即可成房。注意：①無認證，同區網
任何人可加入——信任模型＝災難情境的區網，不要暴露到公網；②無清理 API，**每場會話用新房名**
（重用舊房名會吃到先前會話的信號回放）。

**離線邀請碼**：`createOfflineInvite()` 產 offer 酬載（單張 QR 裝得下，實測 ~630 bytes）→
對方 `acceptOfflineInvite(酬載)` 產 answer 酬載 → 你 `invite.complete(answer)`，兩端各得一條
`OfflineLink` 雙工通道。全程零伺服器；回程管道就是第二張 QR。誠實邊界：此通道是 DTLS 傳輸
加密、尚無房間金鑰 E2EE（沒有 mesh 就沒有 keyx）；「面對面互掃」即身分驗證；持酬載者皆可
完成配對（與「一張碼全村可用」同一信任模型）。
