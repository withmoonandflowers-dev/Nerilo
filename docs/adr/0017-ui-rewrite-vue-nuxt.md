...
6. 遷移窗回歸鎖 `tests/e2e-vue/migration-window.spec.ts`（`@vue-stable`）持續為綠
   （Spec 010／ADR-0036：React 線的星型→mesh 遷移窗掉信不修、記誠實邊界，
   收斂承諾就是本切換——此測試證明接班線確實沒有那個窗，缺它切換等於
   無驗證地帶病退役）。

在第 2 至 6 點完成前，Vue CI 只證明「接班版沒有持續腐壞」，不代表已授權切 production。
...
- docs/DEVELOPMENT.md 的技術棧描述需在切換時更新（現況描述本已與事實不符：
