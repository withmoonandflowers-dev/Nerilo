...
| 盲信使寄存協議（存密文、對帳回補） | P4-C · ADR-0024 · `CourierStore`+`CourierService` | 🟢 C.1 儲存經濟學 + C.2 bus 協議 + C.3 真 relay 通道寄存往返 E2E 綠（`relay-connect.spec.ts` 密文原樣取回）；剩 anti-entropy 自動對帳回補（非 pull）+ tombstone 真簽章 |
