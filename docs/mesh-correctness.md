...
「恰好一次」已於 2026-07-05 達標，但 QA 第三輪留下四項已知限制（`docs/QA-REPORT-chat.md` 行 139-155）。這些都是**設計錯了很久之後才會被發現**的類型——重放、遷移窗掉信、金鑰域擴張。此文件固定四項的優先序、檔案錨點與驗收標準，避免每次進場都重新考古。

## 先讀

- `docs/CURRENT-STATUS.md` — 單一事實來源（現況以它為準，本文件描述過時時亦同）。
...
### 4. mesh 群組 E2EE（第二階段）——已收斂（2026-07-18，Spec 012）
...
- TreeKEM/GroupKeyManager 已退役（ADR-0033）；金鑰輪替口徑（在籍者可解全歷史＝刻意取捨）記於 `docs/THREAT_MODEL.md`。
...
- nuxt 釘 4.4.2，不升。
- 不在本文件內做 Vue 切換門檻（ADR-0017）的事——那是另一條線。
