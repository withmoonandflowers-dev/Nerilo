# 測試強化守則：讓改動穩定、覆蓋高、不弄壞既有

本專案是分散式 + 密碼學系統，最貴的錯是**改動弄壞了正在運作的路徑**（本專案已發生兩次：perfect negotiation 弄壞首次連線、看門狗重連不穩定）。覆蓋率會說謊（行數跑過 ≠ 測試抓得到 bug）。這份文件把業界最有效的做法固化成流程。工具都用標準品（vitest c8 / fast-check / Stryker / Firebase emulator），不重造輪子。
