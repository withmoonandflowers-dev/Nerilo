...
- 關聯：ADR-0003（分層拓撲既有決策：partial mesh／super-node「已有程式碼但未接入」）、ADR-0007（超過 5 人的拓撲列凍結類，「有真實需求再逐案解凍」——本 spec 即解凍案）、ADR-0008（付費層開放大房間的落點）、docs/mesh-correctness.md（殘留清單第 3 項）、docs/CROSS-MACHINE-HANDOFF.md:161-165（Pro 實質權益受「拓撲上限 5 人」限制）
...
- `src/core/mesh/AdaptiveTopologyManager.ts:37-42`：策略邊界 ≤2 direct、≤6 full-mesh、≤20 partial-mesh、>20 super-node。**注意與 docs/DEVELOPMENT.md 拓撲表（3-5 full mesh、6-20 partial mesh）不一致**：程式的 full-mesh 上界是 6，不是 5——「6 人房」按現行程式仍是 full mesh，partial mesh 從第 7 人才開始（Q2）。
...
- [x] **Q2 full-mesh 與 partial-mesh 的切換邊界**：**拍板 (a) 依程式**——6 人仍 full mesh，partial 從第 7 人起；文件（docs/DEVELOPMENT.md Topology Strategy 表、README:19、docs/GOAL-ANALYSIS.md:164-165 的 3-5／6-20 表述）修正對齊程式。
...
- 完成後回填 ADR-0035（partial mesh 接線與只升不降政策的取捨；原編 0033 因與 Spec 009 撞號重編）。
...
- [x] T8 文件收尾：README、GOAL-ANALYSIS、firestore.rules 註解、CROSS-MACHINE-HANDOFF Pro 段、CURRENT-STATUS、ADR-0003/0007 狀態、ADR-0035 回填。
...
- [x] V7 收尾文件：CURRENT-STATUS、QA-REPORT 已知限制、CROSS-MACHINE-HANDOFF Pro 權益段、docs/DEVELOPMENT.md／README／GOAL-ANALYSIS 拓撲表、firestore.rules 註解、ADR-0003／0007 補記、ADR-0035 回填，全數更新。
