...
5. **誠實條款**：不准加長 timeout 硬湊綠；改動讓事情變糟就回退；紅的釘住測試＝停止號誌。（docs/testing-discipline.md）
...
8. **架構決策落 ADR**：只寫看 diff 看不出來的理由與取捨，動詞開頭。spec 的重大取捨完成後要回填 ADR。（ADR-0001、docs/DEVELOPMENT.md）
...
12. **對外文件用語**：禁 em-dash、emoji、口號式排比；引用數字必須核對過真實資料。（文件用語慣例）
13. **commit 風格**：`[<branch>] 動詞開頭一句話 ≤70 字` + 至多 3 行非顯而易見的 context；不加自動產生的署名。（開發文件全域）
