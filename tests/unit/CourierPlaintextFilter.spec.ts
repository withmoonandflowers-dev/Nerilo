...
function rec(over: Partial<GossipMessage> = {}): GossipMessage {
  return {
    roomId: 'r1', senderId: 's1', pubKey: 'pk', seq: 1, timestamp: 1000,
...
describe('推側：reconcile 與 runCourierBackup 不外送明文', () => {
  it('reconcile：成員混合持有明文＋密文＋keyx → 信使只收到密文與 keyx', async () => {
    ...
    const { pushed } = await client.reconcile('r1', buildRoomStore(eligible), new Map(), () => undefined);
  ...
  it('reconcile 防禦層：即使呼叫端漏過濾（digest 含明文），push 前仍被攔下', async () => {
    ...
    const { pushed } = await client.reconcile('r1', buildRoomStore(mixed), new Map(), () => undefined);
